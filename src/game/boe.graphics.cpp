...
//				Draw_Some_Item(roads_gworld, calc_rect(6, 0), terrain_screen_gworld, loc(q,r), 1, 0);
			...
			else if(is_town() && univ.town.testFieldAll<fields::SPECIAL_EXPLORED,fields::SPECIAL_ROAD>(where_draw.x,where_draw.y))
				place_road(q,r,where_draw,true);
...
static bool extend_road_terrain(int x, int y) {
	if(is_out() && univ.out.is_road(x,y))
	   return true;
	if(is_town() && univ.town.testFieldUnchecked<fields::SPECIAL_ROAD>(x,y))
		return true;
